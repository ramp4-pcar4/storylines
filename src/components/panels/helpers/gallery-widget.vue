...
import { nextTick, onMounted, onUnmounted, defineProps, ref, Teleport, Transition } from 'vue';
...
const calculateItemSize = () => {
    ...
    if (galleryWidth <= 150) {
        idealPerRow = Math.min(1, props.maxPerRow);
    } else if (galleryWidth <= 300) {
        idealPerRow = Math.min(2, props.maxPerRow);
    } else if (galleryWidth <= 600) {
        idealPerRow = Math.min(3, props.maxPerRow);
    } else {
        idealPerRow = Math.min(4, props.maxPerRow);
    }

    itemSize.value = (galleryWidth - 8 * idealPerRow) / idealPerRow;
};
